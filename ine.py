...
def refresh_token(original_request):
    ...
    refresh_request = requests.post(
        refresh_endpoint, headers=request_headers, proxies=proxy_config, verify=False
    )
    ...
    if refresh_request.status_code == 200:
        ...
        logger.opt(colors=True).error(
            "There was an issue refreshing the token, doing the login request instead!"
        )
        login()
        return False
...
def fetch_all_courses(to_write_to):
    if os.path.exists(to_write_to):
        logger.opt(colors=True).warning(
            f"{to_write_to} already exists, not fetching the course list again."
        )
        logger.opt(colors=True).warning(f"Delete this file if you want to fetch a fresh list!")

    else:
        all_courses_api_endpoint = "https://content-api.rmotr.com/api/v1/courses?page_size=none&status=published&active=true"
        ...
        if course_data.status_code == 200:
            logger.opt(colors=True).success(
                f"Got the course contents, placing them in {to_write_to}"
            )
            ...
            course_data.status_code == 401
            or course_data.status_code == 403
            or course_data.status_code == 502
        ):
            ...
            logger.opt(colors=True).error(
                "There was some issue in fetching all the courses' metadata"
            )
...
def fetch_user_subscriptions(course_contents, outfile, all_courses_metadata):
    ...
    if user_subs.status_code == 200:
        ...
        logger.opt(colors=True).debug(
            f"Total number of courses accessible to our user: {courses_with_access}"
        )
        ...
        user_subs.status_code == 401 or user_subs.status_code == 403 or user_subs.status_code == 502
    ):
        logger.warning("Sleeping for 10 seconds, server might be rate limiting!")
        sleep(10)
...
def download_video(course_name, content_uuid, complete_path, index):
    logger.opt(colors=True).info(
        f"Fetching video and subs of '{content_uuid}' from '{course_name}'"
    )
    ...
    if video_metadata.status_code == 200:
        ...
        if not os.path.isfile(f"{complete_path}/{video_name}"):
            ...
            for video_details in video_sources[::-1]:
                ...
                # Try and find the best resolution, if not, drop down till we find one
                if video_width == 1920 or video_height == 1080:
                    download_aria2c(f"{complete_path}/{video_name}", video_download_url)
                    break

                elif video_width == 1280 or video_height == 720:
                    download_aria2c(f"{complete_path}/{video_name}", video_download_url)
                    break
        ...
        video_metadata.status_code == 401
        or video_metadata.status_code == 403
        or video_metadata.status_code == 502
    ):
        logger.warning("Sleeping for 10 seconds, server might be rate limiting!")
        sleep(10)
        ...
        logger.opt(colors=True).error(
            f"There was an issue fetching the video {content_uuid} from {course_name}"
        )
    ...
    if initial_quiz_attempt:
        ...
        if not os.path.isfile(quiz_file_name.replace(".txt", "_quiz_notsolved.txt")):
            ...
            if quiz_attempt.status_code == 200:
                ...
                logger.opt(colors=True).warning(
                    f"The quiz attempt with id {quiz_id} has already been made!"
                )

            elif (
                quiz_attempt.status_code == 401
                or quiz_attempt.status_code == 403
                or quiz_attempt.status_code == 502
            ):
                logger.warning("Sleeping for 10 seconds, server might be rate limiting!")
                sleep(10)
                ...
                logger.opt(colors=True).error(
                    f"There was an issue fetching the quiz {content_uuid} with id {quiz_id}"
                )
        ...
        logger.opt(colors=True).error(
            f"There was an issue fetching the quiz {content_uuid} from {course_name}"
        )
...
def download_exercise(course_name, content_uuid, complete_path, index):
    ...
    if exercise_request.status_code == 200:
        ...
        logger.opt(colors=True).error(
            f"There was an issue fetching the exercise {content_uuid} from {course_name}"
        )
...
def download_slide_files(
    slide_url, slide_files_path, els_cdn_cookies, content_uuid, count, extension
):
    ...
    while is_not_end:
        ...
        if not os.path.isfile(slide_file_path):
            if extension == "png" or extension == "woff":
                file_request_url = (
                    f"https://els-cdn.content-api.ine.com/{content_uuid}/data/{slide_file_name}"
                )
...
def download_slide(course_name, content_uuid, complete_path, index):
    ...
    if slides_request.status_code == 200:
        ...
        if not os.path.exists(slide_path):
            logger.opt(colors=True).info(f"Creating directories: '{slide_path}'")
            os.makedirs(slide_path)

        if not os.path.exists(slide_files_path):
            logger.opt(colors=True).info(f"Creating slide directory: '{slide_name}/data/'")
        ...
        for files in slide_files:
            ...
            if slides_files_request.status_code == 200:
                ...
                logger.opt(colors=True).error(
                    f"There was an issue fetching the file {files} of slide {content_uuid} from"
                    f" {course_name}"
                )
        ...
        if not os.path.isfile(f"{slide_path}/index.html"):
            if slide_index.status_code == 200:
                ...
                for files in required_js_files:
                    ...
                    if dl_required_files.status_code == 200:
                        ...
                        write_into_file(
                            f"{slide_path}/{required_fnames}", "w+", required_files_content
                        )
                        logger.opt(colors=True).success(
                            f"Writing '{required_fnames}' to '{slide_path}'"
                        )

                    else:
                        logger.opt(colors=True).error(
                            f"There was an issue fetching the file {required_files_dl_path} from"
                            f" {course_name}"
                        )
                        debug_requests(dl_required_files)

            elif (
                slide_index.status_code == 401
                or slide_index.status_code == 403
                or slide_index.status_code == 502
            ):
                logger.warning("Sleeping for 10 seconds, server might be rate limiting!")
                sleep(10)
                ...
                logger.opt(colors=True).error(
                    f"There was an issue fetching the index file from {slide_url} from"
                    f" {course_name}"
                )
                debug_requests(slide_index)

        else:
            logger.opt(colors=True).warning(f"The file '{slide_path}/index.html' already exists!")

        # Downloading the slide files
        download_slide_files(
            slide_url, slide_files_path, els_cdn_cookies, content_uuid, count=1, extension="css"
        )
        download_slide_files(
            slide_url, slide_files_path, els_cdn_cookies, content_uuid, count=1, extension="js"
        )
        download_slide_files(
            slide_url, slide_files_path, els_cdn_cookies, content_uuid, count=0, extension="png"
        )
        download_slide_files(
            slide_url, slide_files_path, els_cdn_cookies, content_uuid, count=0, extension="woff"
        )

    elif (
        slides_request.status_code == 401
        or slides_request.status_code == 403
        or slides_request.status_code == 502
    ):
        logger.warning("Sleeping for 10 seconds, server might be rate limiting!")
        sleep(10)
        ...
        logger.opt(colors=True).error(
            f"There was an issue fetching the slide {content_uuid} from {course_name}"
        )
...
def download_lab(course_name, content_uuid, complete_path, index):
    ...
    if lab_request.status_code == 200:
        ...
        lab_file_html = f"{complete_path}/{index} - {name}_lab.html"
        lab_file_json = f"{complete_path}/{index} - {name}_lab.json"
        ...
        if not os.path.isfile(lab_file_html):
            if html_description and html_solution:
                ...
                write_into_file(lab_file_html, "w+", f"{html_description}\n{html_solution}")

            elif html_description and not html_solution:
                logger.opt(colors=True).debug(f"Writing the lab file into '{lab_file_html}'")
                write_into_file(lab_file_html, "w+", f"{html_description}")
        ...
        lab_request.status_code == 401
        or lab_request.status_code == 403
        or lab_request.status_code == 502
    ):
        logger.warning("Sleeping for 10 seconds, server might be rate limiting!")
        sleep(10)
        ...
        logger.opt(colors=True).error(
            f"The lab {content_uuid} of {course_name} doesn't exist! lmao.."
        )
        debug_requests(lab_request)

    else:
        logger.opt(colors=True).error(
            f"There was an issue fetching the lab {content_uuid} from {course_name}"
        )
        debug_requests(lab_request)
    ...
    if len(files_uuids) != 0:
        for files in files_uuids:
            ...
            if slides_files_request.status_code == 200:
                ...
                logger.opt(colors=True).error(
                    f"There was an issue fetching the file {files} of {name}"
                )
...
def main():
    ...
    if any(vars(args).values()):
        ...
        user_subscriptions = fetch_user_subscriptions(
            all_course_contents, courses_with_access, all_courses_metadata
        )
